...
if [[ ${IFS_RUNTIME_ENV:-unset} == "unset" ]]; then
  ...
  module_load Boost/1.88.0-cpeCray-25.03
...
module_load cray-dsmml/0.3.1

### Handling of "magic" cray modules
# 1) Load the cray modules
module_load cray-libsci/25.03.0
module_load cray-fftw/3.3.10.10
module_load cray-hdf5/1.12.2.11
module_load cray-netcdf/4.9.0.11
# 2) Store variables to locate the packages

# IFS-4903: Use the non-OpenMP libsci library to avoid linking issues with
# serial and threaded libsci libraries being used at the same time.
...
# IFS-4903: Use the broadwell code path in the libsci library. Using the
# native ones can cause spurious floating point exceptions in some LAPACK calls.
# The reason is unknown at the moment, might be some AVX instructions.
